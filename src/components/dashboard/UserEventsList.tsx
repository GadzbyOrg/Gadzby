...
export function UserEventsList({ events }: Props) {
    ...
    const activeEvents = events.filter(e => e.status === 'OPEN' || e.status === 'DRAFT');
    const closedEvents = events.filter(e => e.status === 'CLOSED' || e.status === 'ARCHIVED');
    ...
    return (
        <div className="mt-8">
            <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">Vos Manips / Événements</h3>
            </div>
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    {activeEvents.length === 0 && (
                        <p className="text-gray-500 text-sm italic">Aucun événement en cours.</p>
                    )}
                </div>
                ...
                {closedEvents.length > 0 && (
                    <div className="border-t border-dark-800 pt-4">
                        ...
                        >
                            {showClosed ? <IconChevronUp size={16} /> : <IconChevronDown size={16} />}
                            {showClosed ? 'Masquer l\'historique' : `Voir l'historique (${closedEvents.length})`}
                        </button>
        ...
        <div className="rounded-xl border border-dark-800 bg-dark-900 p-4 flex flex-col gap-2 transition-transform hover:scale-[1.02]">
             <div className="flex justify-between items-start">
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-medium uppercase border ${
                    event.status === 'OPEN' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 
            ...
            <p className="text-sm text-gray-400 line-clamp-2 min-h-[2.5em]">{event.description || "Pas de description"}</p>
                ...
                <div className="text-sm font-medium text-white">
                    {event.mySpending > 0 ? (
                        <span className="text-rose-400">
                            -{(event.mySpending / 100).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                        </span>
                    ) : (
