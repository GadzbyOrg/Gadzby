		...
		<>
			...
			<Drawer.Root open={isMenuOpen} onOpenChange={setIsMenuOpen}>
				<Drawer.Portal>
					...
					<Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 bg-surface-900 border-t border-border rounded-t-[20px] shadow-2xl md:hidden outline-none flex flex-col mt-24 max-h-[85vh] pb-[env(safe-area-inset-bottom)]">
						<div className="flex justify-center p-2 rounded-t-[20px] bg-surface-950/50">
							<div className="w-12 h-1.5 rounded-full bg-elevated" />
						</div>

						<div className="flex items-center justify-between px-5 py-3 border-b border-border bg-surface-950">
							<Drawer.Title className="text-sm font-bold uppercase tracking-wider text-fg">
								{activeMain}
							</Drawer.Title>
							<Drawer.Close asChild>
								<button className="p-1 rounded-full text-fg-muted hover:bg-elevated hover:text-fg transition-colors">
									<IconX size={20} />
								</button>
							</Drawer.Close>
						</div>

						<div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-1 bg-surface-900 pb-6">
								...
								if ("type" in item && item.type === "dropdown") {
									...
									return (
										<div key={item.label} className="flex flex-col">
											...
											{isOpen && (
												<div className="ml-4 mt-1 mb-2 flex flex-col gap-1 border-l-2 border-border pl-3">
