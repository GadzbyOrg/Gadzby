...
import { transferMoneyAction, TransferState } from "@/features/transactions/actions";
import { searchUsersPublicAction } from "@/features/users/actions";
import { IconCurrencyEuro, IconSearch, IconCheck, IconSelector } from "@tabler/icons-react";
...
export function TransferForm({ balance }: { balance: number }) {
    const [state, action, isPending] = useActionState(transferMoneyAction, initialState);
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-6">
                ...
                <div className="rounded-xl border border-primary-900/30 bg-gradient-to-br from-primary-900/10 to-transparent p-6">
                    <h3 className="text-sm font-medium text-gray-400">Votre solde actuel</h3>
                ...
                <div className="rounded-xl border border-dark-800 bg-dark-900 p-6 shadow-sm">
                    <h2 className="text-lg font-semibold text-white mb-6">Effectuer un virement</h2>
                    ...
                    <form action={action} className="space-y-6">
                                ...
                                <div className="flex items-center justify-between p-3 rounded-lg border border-dark-700 bg-dark-800 text-white">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-xs text-gray-400">
                                            {selectedUser.username} {selectedUser.bucque && `- ${selectedUser.bucque}`}
                                        </span>
                                    </div>
                                    <button 
                                        type="button" 
                        ...
                        <div className="space-y-2">
                            <label htmlFor="amount" className="text-sm font-medium text-gray-300">Montant (€)</label>
                            <div className="relative">
                        ...
                        {/* Description */}
                        <div className="space-y-2">
                            <label htmlFor="description" className="text-sm font-medium text-gray-300">Message (optionnel)</label>
                            <textarea 
                                id="description" 
                                name="description"
