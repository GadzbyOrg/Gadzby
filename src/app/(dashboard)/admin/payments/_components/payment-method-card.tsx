...
import { togglePaymentMethod, updatePaymentMethodConfig } from "@/features/payments/admin";
import { IconLoader2, IconSettings, IconPlus, IconTrash, IconX } from "@tabler/icons-react";
...
export function PaymentMethodCard({ method }: { method: PaymentMethod }) {
  ...
  const [configFields, setConfigFields] = useState<{ id: string; key: string; value: string }[]>([]);
  ...
  const handleSave = async () => {
    ...
    try {
      await updatePaymentMethodConfig(method.id, fees, newConfig);
      setIsEditing(false);
    } catch (e) {
  ...
  const addField = () => {
      setConfigFields([...configFields, { id: crypto.randomUUID(), key: "", value: "" }]);
  };
  ...
  const updateField = (id: string, field: "key" | "value", newValue: string) => {
      setConfigFields(configFields.map(f => 
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
                 <h3 className="text-lg font-bold tracking-tight text-white">{method.name}</h3>
                 <span className="text-xs text-dark-700 bg-dark-950 px-2 py-0.5 rounded font-mono border border-dark-800">
                    {method.slug}
                 </span>
            </div>
            {method.description && (
                <p className="text-sm text-gray-400 mt-1 line-clamp-2">{method.description}</p>
            )}
            ...
            <div className="mt-4 flex items-center gap-3">
                 ...
                 {!isEditing && (
                    <div className="text-sm font-medium text-gray-300">
                        {method.fees.fixed > 0 ? (method.fees.fixed / 100).toFixed(2) + "€" : "Gratuit"} 
        ...
        {isEditing && (
          <div className="space-y-4 pt-4 border-t border-dark-800 animate-in slide-in-from-top-2 duration-200">
             ...
             <div className="flex items-center justify-between p-3 rounded-lg bg-dark-950 border border-dark-800">
                <span className="text-sm font-medium text-gray-300">État du service</span>
                <button 
             ...
             <div className="grid grid-cols-2 gap-4">
               <div>
                 <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Frais fixes (cts)</label>
                 <div className="relative mt-1">
                    <input 
                    type="number" 
                    value={fees.fixed}
                    onChange={(e) => setFees({ ...fees, fixed: Number(e.target.value) })}
                    ...
                    placeholder="0"
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-500">cts</span>
                 </div>
               </div>
               <div>
                 <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Frais (%)</label>
                 <div className="relative mt-1">
                    <input 
                    type="number" 
                    step="0.01"
                    value={fees.percentage}
                    onChange={(e) => setFees({ ...fees, percentage: Number(e.target.value) })}
                    className="w-full rounded-lg border border-dark-700 bg-dark-950 p-2 text-sm text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none transition-all"
                    placeholder="0.00"
                    />
             ...
             <div className="space-y-2">
                <label className="text-xs font-medium text-gray-500 uppercase tracking-wider flex items-center justify-between">
                    Configuration
                    <span className="text-[10px] text-gray-600">{configFields.length} variables</span>
                </label>
