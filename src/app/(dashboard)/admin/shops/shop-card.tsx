...
export function ShopCard({ shop }: ShopCardProps) {
    ...
    return (
        <div className={`bg-dark-900 border rounded-xl overflow-hidden transition-colors group ${shop.isActive ? 'border-dark-800 hover:border-primary-900/50' : 'border-dark-800 opacity-75'}`}>
            <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                    <div className={`p-3 rounded-lg transition-colors ${shop.isActive ? 'bg-primary-900/20 text-primary-400 group-hover:bg-primary-600 group-hover:text-white' : 'bg-dark-800 text-gray-500'}`}>
                        <IconBuildingStore size={24} stroke={1.5} />
                    </div>
                    <button 
                        onClick={handleToggleStatus}
                        disabled={loading}
                        className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-600 focus-visible:ring-offset-2 focus-visible:ring-offset-dark-900 ${shop.isActive ? 'bg-emerald-600' : 'bg-dark-700'}`}
                    >
