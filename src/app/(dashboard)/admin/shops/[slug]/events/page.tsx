...
export default async function ShopEventsPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const activeEvents = events.filter(e => e.status === 'DRAFT' || e.status === 'OPEN');
    ...
    return (
        <div className="flex flex-col gap-6 p-4 md:p-8">
            <div className="flex justify-between items-center">
                ...
                <Link
                    href={`/admin/shops/${slug}/events/create`}
            ...
            <div className="flex flex-col gap-8">
             ...
             {events.length === 0 && (
                <p className="text-gray-500 text-center py-8">Aucun événement planifié.</p>
            )}
        </div>
    );
}
                ...
                <div className="flex items-center gap-2 text-gray-500 text-sm mt-2">
                 ...
                 <div className="flex gap-2">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium border ${
                        event.type === 'SHARED_COST' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-orange-500/10 text-orange-400 border-orange-500/20'
                    }`}>
                        {event.type}
                    </span>
                </div>
