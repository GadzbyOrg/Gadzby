        ...
        startTransition(async () => {
            try {
                ...
                if (initialData) {
                    ...
                    toast({ title: 'Succès', description: 'Événement mis à jour', variant: 'default' });
                } else {
                    await createEvent(shopId, payload);
                    toast({ title: 'Succès', description: 'Événement créé', variant: 'default' });
        ...
        <div className="bg-dark-800 border border-dark-700 p-6 rounded-lg max-w-2xl">
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                    <label className="text-sm font-medium text-gray-300">Nom de l'événement</label>
                    <input
                        type="text"
                        placeholder="Ex: Voyage Berlin 2025"
                ...
                <div className="grid grid-cols-2 gap-4">
                     <div className="flex flex-col gap-1">
                        ...
                        />
                         {form.formState.errors.startDate && <span className="text-red-400 text-xs">{form.formState.errors.startDate.message}</span>}
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-300">Date de fin (Optionnel)</label>
                        <input
                            type="date"
                            className="bg-dark-900 border border-dark-700 rounded-md p-2 text-white focus:outline-none focus:border-primary-500"
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <select
                        ...
                        {...form.register('type')}
                    >
                        <option value="SHARED_COST">Coûts Partagés (Acompte + Justif)</option>
                ...
                {type === 'SHARED_COST' && (
                     <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-300">Acompte (en €)</label>
                        <input
                            type="number"
                            step="0.01"
                            min="0"
                ...
                <div className="flex items-center gap-2 mt-2">
                    ...
                    />
                    <label htmlFor="allowSelfRegistration" className="text-sm text-gray-300">
                ...
                <div className="flex justify-end gap-3 mt-4">
                    ...
                    >
                        {isPending ? 'Chargement...' : initialData ? 'Mettre à jour' : 'Créer l\'événement'}
                    </button>
                </div>
            </form>
        </div>
