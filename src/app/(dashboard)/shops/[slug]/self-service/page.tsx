import { getShopBySlug, getShopDetailsForMember } from "@/features/shops/actions";
...
export default async function ShopSelfServicePage({
    ...
    if (session) {
        ...
        if (memberData) {
            ...
            isManager = ["VP", "TRESORIER", "PRESIDENT", "RESPO"].includes(membership.role) || session.role === "ADMIN";
        }
    }
    ...
    if (isEnabled) {
        ...
        const { getSelfServiceProducts } = await import("@/features/shops/products");
        ...
        return (
            <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center min-h-[50vh] text-center space-y-4">
                ...
                <div className="bg-dark-900 border border-dark-800 rounded-3xl p-12 max-w-lg w-full">
                    ...
                    <h2 className="text-2xl font-bold text-white mb-2">Self-Service Fermé</h2>
                    <p className="text-gray-400">
                        Le self-service du shop {shop.name} est actuellement fermé ou indisponible.
                    </p>
                    <Link 
                        href="/shops"
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <header className="border-b border-dark-800 pb-6 mb-6">
                ...
                <p className="text-gray-400">
                    Bienvenue au shop {shop.name}. Commandez en toute autonomie.
                </p>
            </header>
            ...
            {!isEnabled && isManager && (
                <div className="mb-8 p-4 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-orange-400 font-medium">Self-Service Désactivé</h3>
                        <p className="text-sm text-orange-400/80">
                            Ce shop est fermé au public. Vous voyez cette page car vous êtes manager ou administrateur. 
                            <Link href={`/shops/${slug}/manage/settings`} className="underline ml-1 hover:text-orange-300">
