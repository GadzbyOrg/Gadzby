...
export function TopCustomersCard({ data, loading }: TopCustomersCardProps) {
	if (loading) {
		return (
	...
	return (
		<div className="bg-surface-900 p-6 rounded-xl border border-border flex flex-col h-full">
			<h3 className="text-lg font-medium text-fg mb-4">Meilleurs Clients</h3>
			<div className="flex-1 overflow-auto space-y-4 pr-1">
				{data.length === 0 ? (
					<p className="text-fg-subtle text-sm">Aucun client</p>
				) : (
					data.map((item, index) => (
						<div
							key={item.userId}
						...
						>
							...
							<div className="flex-1 min-w-0">
								...
								<div className="flex items-center gap-1.5 mt-1">
									...
									<span className="text-[10px] uppercase tracking-wider font-bold text-fg-subtle">achats</span>
								</div>
							</div>
							<div className="text-right shrink-0">
								...
								<p className="text-[10px] mt-1 uppercase tracking-wider font-bold text-fg-subtle">dépensés</p>
							</div>
						</div>
					))
				)}
