...
describe("formatPeriodLabel", () => {
	it("formats month period as abbreviated month and year in French", () => {
		const label = formatPeriodLabel("2026-05", "month");
		expect(label).toMatch(/mai/i);
		expect(label).toMatch(/2026/);
	});

	it("formats day period as DD/MM", () => {
		const label = formatPeriodLabel("2026-04-27", "day");
		expect(label).toBe("27/04");
	});

	it("formats week period (Monday date) as DD/MM", () => {
		const label = formatPeriodLabel("2026-04-27", "week");
		expect(label).toBe("27/04");
	});
});
