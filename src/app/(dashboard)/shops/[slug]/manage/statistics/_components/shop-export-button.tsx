...
export function ShopExportButton({ slug }: { slug: string }) {
    ...
    const handleExport = () => {
        startExport(async () => {
            ...
            const res = await exportShopTransactionsAction(slug, search, "ALL", sort, startDate, endDate);
            ...
            if (res.data) {
                ...
                XLSX.writeFile(workbook, `${slug}_transactions_${new Date().toISOString().split('T')[0]}.xlsx`);
            }
        });
    };
