			...
			<div className="flex-1 overflow-auto space-y-4 pr-1">
				{data.length === 0 ? (
					...
					data.map((item, index) => (
						...
						>
							<div
								className={`flex-shrink-0 flex items-center justify-center font-mono text-xs font-bold w-6 h-6 rounded-md ${type === "top"
							...
							<div className="flex-1 min-w-0">
								...
								<div className="flex items-center gap-2 mt-1">
									<span className="text-[10px] uppercase tracking-wider font-bold text-fg-subtle">Stock</span>
									<span className={`text-xs font-mono ${item.product.stock <= 5 ? "text-red-400" : "text-fg-muted"
										}`}>
							...
							<div className="text-right flex flex-col items-end shrink-0">
								<div className="flex items-center gap-1.5 leading-none">
									<span className="text-sm font-bold text-fg font-mono">{item.totalQuantity}</span>
									<span className="text-[10px] uppercase tracking-wider font-bold text-fg-subtle">vendu{item.totalQuantity > 1 ? 's' : ''}</span>
								</div>
								<div className="mt-1 px-1.5 py-0.5 rounded bg-accent-500/10 border border-accent-500/20 leading-none">
									<span className="text-[11px] font-bold text-accent-400 font-mono tracking-tight">
