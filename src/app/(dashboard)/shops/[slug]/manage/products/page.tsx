...
export default async function ShopProductsPage({
    ...
    const sortBy = typeof resolvedSearchParams.sortBy === 'string' ? resolvedSearchParams.sortBy as 'name' | 'price' | 'stock' : undefined;
    const sortOrder = typeof resolvedSearchParams.sortOrder === 'string' ? resolvedSearchParams.sortOrder as 'asc' | 'desc' : undefined;
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
            <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
                <Link href={`/shops/${slug}/manage`} className="hover:text-white transition-colors">
                    ← Retour à la gestion
                </Link>
                <span>/</span>
                            ...
                            {products && products.length > 0 ? (
                                products.map((product) => (
                                    <tr key={product.id} className="hover:bg-dark-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 ${product.stock <= 5 ? 'text-red-400' : 'text-green-400'}`}>
                                                <span className={`w-1.5 h-1.5 rounded-full ${product.stock <= 5 ? 'bg-red-400' : 'bg-green-400'}`}></span>
                                                {product.stock}
                                            </span>
                                        </td>
