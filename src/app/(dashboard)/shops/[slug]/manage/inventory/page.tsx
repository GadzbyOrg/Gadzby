...
import DeleteAuditButton from "./_components/DeleteAuditButton";
...
export default async function ShopInventoryPage({
	...
	const calculateTotalDiscrepancy = (
		items: { difference: number; product: { price: number } }[],
	) => {
		return (
			items.reduce(
				(acc, item) => acc + item.difference * item.product.price,
				0,
			) / 100
		);
	};
										...
										{audits.map((audit) => {
											...
											return (
												...
												>
													...
													<td
														className={`px-6 py-4 text-right font-mono ${discrepancy < 0 ? "text-red-400" : discrepancy > 0 ? "text-green-400" : "text-fg-subtle"}`}
													>
														{discrepancy > 0 ? "+" : ""}
														{discrepancy.toFixed(2)}€
													</td>
