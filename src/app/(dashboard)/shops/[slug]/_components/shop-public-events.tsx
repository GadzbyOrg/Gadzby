...
import { IconCalendarPlus, IconCheck, IconLoader2, IconLogout } from '@tabler/icons-react';
    ...
    const handleJoin = (eventId: string) => {
        startTransition(async () => {
            try {
                ...
                toast({ title: 'Succès', description: 'Inscription validée !', variant: 'default' });
            } catch (error: any) {
                toast({ title: 'Erreur', description: error.message || 'Impossible de rejoindre', variant: 'destructive' });
            }
        });
    };
        ...
        if (!confirm('Se désinscrire de cet événement ?')) return;
         startTransition(async () => {
            try {
                ...
                toast({ title: 'Succès', description: 'Désinscription effectuée', variant: 'default' });
            } catch (error: any) {
                toast({ title: 'Erreur', description: error.message || 'Impossible de quitter', variant: 'destructive' });
            }
        });
    }
    ...
    return (
        <div className="mb-8 p-6 bg-dark-900 border border-dark-800 rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {events.map(event => (
                    <div key={event.id} className="flex flex-col gap-3 p-4 rounded-xl bg-dark-800 border border-dark-700">
                        <div>
                            ...
                            <div className="text-xs text-gray-500 mt-2">
                                {new Date(event.startDate).toLocaleDateString()} • {event.type === 'SHARED_COST' ? 'Coûts Partagés' : 'Commercial'}
                            </div>
                        </div>
                        ...
                        <div className="mt-auto pt-2">
                            {event.isJoined ? (
                                ...
                                >
                                    {isPending ? <IconLoader2 className="animate-spin" size={16} /> : <IconLogout size={16} />}
                                    Se désinscrire
                                </button>
                            ) : (
                                <button
                                ...
                                >
                                    {isPending ? <IconLoader2 className="animate-spin" size={16} /> : 'Rejoindre'}
                                </button>
                            )}
                        </div>
                    </div>
