        ...
        return (
            <div className="rounded-xl border border-dark-700 bg-dark-800 p-4">
                    ...
                    <div className="flex items-center gap-4">
                         ...
                         <div className="h-12 w-12 rounded-full overflow-hidden border border-dark-600 bg-dark-700 flex items-center justify-center">
                            {selectedClient.image ? (
                                <img src={selectedClient.image} alt={selectedClient.username} className="h-full w-full object-cover" />
                            ) : (
                                <span className="text-sm font-bold text-gray-400">{selectedClient.username.substring(0,2).toUpperCase()}</span>
                            )}
                         </div>

                        <div>
                            <div className="flex items-center gap-2">
                                <span className="font-semibold text-white text-lg">{selectedClient.username}</span>
                                <span className="text-xs text-gray-400">({selectedClient.bucque})</span>
                            </div>
                 ...
                 <div className="mt-4 flex items-center justify-between rounded-lg bg-dark-900 p-3">
                    ...
                    <span className={`text-xl font-mono font-bold ${selectedClient.balance < 0 ? 'text-red-400' : 'text-green-400'}`}>
