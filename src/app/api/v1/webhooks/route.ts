...
const createWebhookSchema = z.object({
	url: z.string().url("Must be a valid HTTPS URL").regex(/^https:\/\//, "URL must use HTTPS"),
	events: z.array(z.enum(["shop.purchase.created"])).min(1),
});

export async function GET(req: NextRequest) {
	const authRes = await validateApiKey(req);
	if (!authRes.success) return NextResponse.json({ error: authRes.error }, { status: authRes.status });

	try {
		const webhooks = await db.query.apiWebhooks.findMany({
			where: eq(apiWebhooks.apiKeyId, authRes.keyRecord!.id),
			columns: { id: true, url: true, events: true, isActive: true, createdAt: true },
		});
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const [webhook] = await db.insert(apiWebhooks).values({
