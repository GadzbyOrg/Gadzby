...
import { db } from "@/db";
import { events, eventTypeEnum, eventStatusEnum, eventRevenues } from "@/db/schema/events";
...
async function verifyShopPermission(shopId: string, action: "canManageProducts" | "canViewStats") {
...
export async function createEvent(shopId: string, data: {
    name: string;
    description?: string;
    startDate: Date;
    endDate?: Date;
...
export async function updateEvent(shopId: string, eventId: string, data: Partial<typeof events.$inferInsert>) {
    const session = await verifyShopPermission(shopId, "canManageProducts");
    if (!session) throw new Error("Unauthorized");
...
export async function activateEvent(shopId: string, eventId: string) {
    ...
    if (acompte > 0) {
        ...
        if (participantsIds.length > 0) {
            ...
            // Identify unpaid
            const unpaidParticipants = event.participants.filter(p => !paidUserIds.has(p.userId));
            ...
            if (unpaidParticipants.length > 0) {
                ...
                const usersData = await db.query.users.findMany({
                    ...
                    columns: { id: true, balance: true, isAsleep: true, prenom: true, nom: true }
                });

                const insufficientFundsUsers = usersData.filter(u => u.balance < acompte);
                let warningMessage: string | undefined;

                if (insufficientFundsUsers.length > 0) {
                     const names = insufficientFundsUsers.map(u => `${u.prenom} ${u.nom}`).join(", ");
                ...
                return { success: true, warning: warningMessage };
                 ...
                 await db.update(events).set({ status: "OPEN" }).where(eq(events.id, eventId));
            }
        } else {
...
export async function joinEvent(eventId: string, userId?: string) {
    ...
    if (event.type === 'COMMERCIAL') {
        throw new Error("Les inscriptions sont désactivées pour les événements commerciaux.");
    }

    // If joining self, check if allowed
    if (targetUserId === session.userId) {
        if (!event.allowSelfRegistration) {
             const authorized = await verifyShopPermission(event.shopId, "canManageProducts");
    ...
    if (acompte > 0) {
        ...
        if (user.balance < acompte) throw new Error("Solde insuffisant pour l'acompte");
...
export async function updateParticipant(eventId: string, userId: string, data: { weight?: number, status?: "PENDING" | "APPROVED" | "REJECTED" }) {
    const session = await verifySession();
    if (!session) throw new Error("Unauthorized");
...
export async function importParticipants(eventId: string, criteria: { promss?: string, bucque?: string }) {
    const session = await verifySession();
    if (!session) throw new Error("Unauthorized");
    ...
    if (event.type === 'COMMERCIAL') {
        throw new Error("Impossible d'importer des participants pour un événement commercial.");
    }
...
export async function importParticipantsFromList(eventId: string, data: { identifier: string, weight?: number }[]) {
    const session = await verifySession();
    if (!session) throw new Error("Unauthorized");
    ...
    const uniqueIdentifiers = Array.from(new Set(data.map(d => d.identifier).filter(Boolean)));
...
export async function getEventStats(eventId: string) {
    ...
    const totalExpenses = Number(directExpensesResult[0]?.total || 0) + Number(splitExpensesResult[0]?.total || 0);
...
export async function calculateShopRevenueForPeriod(shopId: string, startDate: Date, endDate: Date) {
    ...
    // Return absolute value (positive revenue)
...
export async function previewSettlement(eventId: string) {
    ...
    // Total Weights
    const totalWeight = event.participants.reduce((sum, p) => sum + (p.weight || 1), 0);
...
export async function linkProductToEvent(shopId: string, eventId: string, productId: string) {
    const session = await verifyShopPermission(shopId, "canManageProducts");
    if (!session) throw new Error("Unauthorized");
    ...
    if (event.type === 'SHARED_COST') {
        throw new Error("Impossible de lier des produits à un événement à coûts partagés.");
    }
...
export async function linkProductsToEvent(shopId: string, eventId: string, productIds: string[]) {
    const session = await verifyShopPermission(shopId, "canManageProducts");
    if (!session) throw new Error("Unauthorized");
...
export async function unlinkProductFromEvent(shopId: string, eventId: string, productId: string) {
    const session = await verifyShopPermission(shopId, "canManageProducts");
    if (!session) throw new Error("Unauthorized");
...
export async function getAvailableProducts(shopId: string) {
    return await db.query.products.findMany({
        where: and(eq(products.shopId, shopId), eq(products.isArchived, false), sql`${products.eventId} IS NULL`),
        orderBy: [desc(products.name)]
    }); 
}

export async function linkExpenseToEvent(shopId: string, eventId: string, expenseId: string) {
    const session = await verifyShopPermission(shopId, "canViewStats"); 
    if (!session) throw new Error("Unauthorized");

    await db.update(shopExpenses)
        .set({ eventId })
...
export async function unlinkExpenseFromEvent(shopId: string, eventId: string, expenseId: string) {
    const session = await verifyShopPermission(shopId, "canViewStats");
    if (!session) throw new Error("Unauthorized");
...
export async function getAvailableExpenses(shopId: string) {
    return await db.query.shopExpenses.findMany({
        where: and(eq(shopExpenses.shopId, shopId), sql`${shopExpenses.eventId} IS NULL`),
        orderBy: [desc(shopExpenses.date)],
        limit: 50 
    });
}
...
export async function getShopPublicEvents(shopId: string, currentUserId?: string) {
    ...
    const myParticipations = await db.query.eventParticipants.findMany({
        where: and(
            ...
            inArray(eventParticipants.eventId, publicEvents.map(e => e.id))
        )
    });
...
export async function createEventRevenue(shopId: string, eventId: string, data: { amount: number, description: string }) {
    const session = await verifyShopPermission(shopId, "canManageProducts");
    if (!session) throw new Error("Unauthorized");
...
export async function deleteEventRevenue(shopId: string, eventId: string, revenueId: string) {
    const session = await verifyShopPermission(shopId, "canManageProducts");
    if (!session) throw new Error("Unauthorized");
...
export async function splitExpense(shopId: string, eventId: string, expenseId: string, amount: number) {
     const session = await verifyShopPermission(shopId, "canViewStats"); 
     if (!session) throw new Error("Unauthorized");
...
export async function deleteExpenseSplit(shopId: string, eventId: string, splitId: string) {
    const session = await verifyShopPermission(shopId, "canViewStats");
    if (!session) throw new Error("Unauthorized");
