...
import { db } from "@/db";
import { inventoryAudits, inventoryAuditItems, products, shops, shopUsers, productRestocks, transactions } from "@/db/schema";
...
async function checkShopPermission(shopSlug: string) {
    ...
    // TODO: finer grained permissions for inventory? For now, allow members.
    return { session, isAuthorized: true, shop, membership };
}

export async function restockProduct(shopSlug: string, productId: string, quantity: number) {
    ...
    if (perm.session.permissions.includes("ADMIN_ACCESS") || perm.session.permissions.includes("MANAGE_SHOPS")) {
        ...
        // @ts-ignore
        const canManage = (perm.membership?.role === "GRIPSS" || perm.shop.permissions?.[perm.membership?.role.toLowerCase()]?.canManageProducts);
...
export async function updateAuditItem(shopSlug: string, auditId: string, itemId: string, actualStock: number) {
    const perm = await checkShopPermission(shopSlug);
    if (!perm) return { error: "Non autorisé" };
...
export async function completeInventoryAudit(shopSlug: string, auditId: string) {
    const perm = await checkShopPermission(shopSlug);
    if (!perm) return { error: "Non autorisé" };
    ...
    try {
        ...
        if (audit.status === 'COMPLETED') return { error: "Inventaire déjà validé" };



        await db.transaction(async (tx) => {
             // Mark audit as completed
            await tx.update(inventoryAudits)
